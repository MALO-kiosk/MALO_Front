...
export default function StampInput({ onNext, onSkip }: StampInputProps) {
  ...
  const handleKeyPress = (val: string) => {
    ...
    if (phoneNumber.length >= 11) return; // 최대 길이 제한
    setPhoneNumber(prev => prev + val);
  };

  // 번호 포맷팅 (010-0000-0000)
    ...
    <div className="complete-page stamp-input-page">
      ...
      <div className="complete-card">
        ...
        <div className="keypad-grid">
          ...
          <button className="key-btn" onClick={() => setPhoneNumber('010')}>010</button>
          <button className="key-btn" onClick={() => handleKeyPress('0')}>0</button>
          <button className="key-btn back-btn" onClick={() => handleKeyPress('←')}>
